...
const Projects = () => {
      ...
      <div className="flex-shrink-0 w-full md:w-1/2 flex flex-col rounded-xl shadow-2xl">
          ...
          {["galeria", "caracteristicas"].map((t) => (
            <div
              ...
              onClick={() =>
                setTab((prev) => ({
                  ...prev,
                  [tipoCasa]: t as "galeria" | "caracteristicas",
                }))
              }
              className={`cursor-pointer flex-1 text-center py-3 font-title md:text-base relative text-sm
                ${
                  tab[tipoCasa] === t
                    ? "text-[#0F70B7] font-semibold"
                    : "text-[#6a7282]"
                }`}
            >
              {t === "galeria" ? "Galería" : "Características"}
              {tab[tipoCasa] === t && (
                <span className="absolute left-0 bottom-0 w-full h-[2px] bg-[#0F70B7]" />
              )}
            </div>
          ))}
        </div>
        ...
        {tab[tipoCasa] === "galeria" && (
          <div className="relative flex flex-col items-center justify-center p-3">
            ...
            <span className="mt-2 text-sm text-[#4a5565] font-title">
              {data.imagenes[currentIdx].title}
            </span>
            <div className="flex gap-2 mt-2">
              {data.imagenes.map((_, index) => (
                <div
                  key={index}
                  onClick={() =>
                    setCurrentImage((prev) => ({ ...prev, [tipoKey]: index }))
                  }
                      ...
                      className={`h-5 w-5 mr-5 transition-transform
                        ${
                          activeAccordion[tipoKey] === item.titulo
                            ? "rotate-180 text-[#0F70B7]"
                            : "text-[#99a1af]"
                        }`}
                    />
                  </button>
                  {activeAccordion[tipoKey] === item.titulo && (
                    <div className="mt-2 text-sm text-[#4a5565] transition-all  text-start px-5">
                      {item.descripcion}
                    </div>
                  )}
                </li>
  ...
  const subtipoSeleccionado = tipo1Piso.subtipo?.find(
    (s) => s.id === subtipo1Piso
  )!;
  ...
  return (
    <section
      ...
      data-aos="fade-up"
      ref={sectionRef}
      className="px-8 gap-8 lg:pt-8 pt-2 pb-12 text-center bg-[#ffffff]"
    >
            ...
            >
              <div className="flex-1 flex flex-col items-start gap-3 text-start">
                ...
                <span className="border-b w-full pb-4 text-[#364153] border-[#d1d5dc] font-title">
                  Casa de: {tipo2Pisos.areaConstruida} | Terreno de:{" "}
                  {tipo2Pisos.areaTerreno}
                </span>
                ...
                <div className="flex justify-between w-full gap-4 pb-2">
                  <div className="flex flex-col">
                    ...
                    <span className="font-numbers text-sm md:text-lg">
                      {tipo2Pisos.cuotaDesde}
                    </span>
          ...
          {tipoCasa === "1piso" && tipo1Piso && subtipoSeleccionado && (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row justify-between gap-6">
                <div className="flex-1 flex flex-col items-start gap-3 text-start">
                  <span className="font-title font-bold text-2xl">
                    {subtipoSeleccionado.titulo}
                  </span>
                  <span className="border-b w-full pb-4 text-[#364153] border-[#d1d5dc] font-title">
                    Casa de: {subtipoSeleccionado.areaConstruida} | Terreno de:{" "}
                    {subtipoSeleccionado.areaTerreno}
                  </span>
                  <div className="flex justify-between w-full text-[#1e2939] border-b pb-4 border-[#d1d5dc]">
                    <span className="font-title">Precio al contado desde</span>
                    <span className="font-semibold font-numbers md:text-2xl text-xl">
                  ...
                  <div className="flex justify-between w-full gap-4 pb-2">
                    <div className="flex flex-col">
                      <span className="pb-2 font-title">Cuota desde</span>
                      <span className="font-numbers text-sm md:text-lg">
                        {subtipoSeleccionado.cuotaDesde}
                      </span>
                      <span className="text-[#0F70B7] border-[#0F70B7] border font-bold text-center py-1 rounded-full w-36 mt-2 text-xs font-title">
                ...
                <TabsGaleriaCaracteristicas
                  data={subtipoSeleccionado}
                  tipoKey={`1piso-${subtipoSeleccionado.id}`}
                />
              </div>
            </motion.div>
          )}
